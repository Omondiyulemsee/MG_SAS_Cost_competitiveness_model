...
#loads
load_1_dict = {
    "load_data_directory": str(ROOT / "input_csv" / "load_data.csv"),
    "MG_results_directory": str(ROOT / "sample_results" / "MG" / "MG.csv"),
    "SAS_results_directory": str(ROOT / "sample_results" / "SAS" / "SAS_combined.csv"),
    "weighted_results_directory": str(ROOT / "sample_results" / "Weighted" / "weighted.csv"),
}
#irradiation data
GHI_data_df = pd.read_csv(ROOT / "input_csv" / "GHI_data.csv", delimiter=";")
#Define the load that you want to calculate costs for
used_data = load_1_dict

Path(load_1_dict["MG_results_directory"]).parent.mkdir(parents=True, exist_ok=True)
Path(load_1_dict["SAS_results_directory"]).parent.mkdir(parents=True, exist_ok=True)
Path(load_1_dict["weighted_results_directory"]).parent.mkdir(parents=True, exist_ok=True)
...
def MG_LCOE_calculator(load, num_of_loads,MG_system_capacities):
    ...
    for capacities in MG_system_capacities:
        ...
        inverter_size = float(np.asarray(load).max()) #inverter should be able to handle the peak power
...
def SAS_LCOE_calculator(load, start_year,
        ...
        if total_load > 0: #remove empty columns
            for capacities in SAS_system_capacities:  # try different system sizes for each load and find min LCOE at LOEP required
                ...
                inverter_size = float(load.to_numpy().max())  # inverter should be able to handle the peak power
